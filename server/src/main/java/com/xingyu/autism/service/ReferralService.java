...
public class ReferralService {
    ...
    public List<HospitalRecommendDto> recommend(double userLat, double userLng, Long childId, String gradeFilter) {
        ...
        List<Hospital> hospitals = new ArrayList<>(mockHospitalDataService.getAllHospitals());
        if (hospitals.isEmpty()) return Collections.emptyList();

        // 构建缓存 Map，后续按 ID 查找详情时避免线性遍历
        Map<Long, Hospital> hospitalCache = new HashMap<>();
        for (Hospital h : hospitals) {
            hospitalCache.put(h.getId(), h);
        }
    ...
    public HospitalDetailDto getHospitalDetail(Long id) {
        // 用 Map 缓存替代线性遍历
        List<Hospital> all = mockHospitalDataService.getAllHospitals();
        Map<Long, Hospital> cache = new HashMap<>();
        for (Hospital h : all) {
            cache.put(h.getId(), h);
        }
        Hospital h = cache.get(id);
    ...
    private double calcDynamicSpecialtyScore(String childAgeRange, String childRisk, Hospital hospital) {
